...
# Load the data set
human<-readRDS("../../../cellranger_results/PDX/K35R-BR284Y/SJ_DIPG_X7_XP1.soupX.adjusted.rds")
human.umi<-human[grep("^GRCh38",rownames(human)),]
rownames(human.umi)<-gsub("GRCh38_","",rownames(human.umi))

colnames(human.umi)<-paste(colnames(human.umi),"SJ_DIPG_X7_XP1",sep="_")

sce<-SingleCellExperiment(assays = list(counts = human.umi))

obj<-new("SingCellaR", sce)
obj@sample_uniq_id<-"SJ_DIPG_X7_XP1"
...
save(obj,file="SJ_DIPG_X7_XP1.soupX.SingCellaR.rdata")
