...
human<-readRDS("../../../cellranger_results/PDX/K35R-XN9L9Y/RCXXB47_XP1.soupX.adjusted.rds")
human.umi<-human[grep("^GRCh38",rownames(human)),]
rownames(human.umi)<-gsub("GRCh38_","",rownames(human.umi))

colnames(human.umi)<-paste(colnames(human.umi),"RCXXB47_XP1_pellet",sep="_")

sce<-SingleCellExperiment(assays = list(counts = human.umi))

obj<-new("SingCellaR", sce)
obj@sample_uniq_id<-"RCXXB47_XP1_pellet"
...
save(obj,file="RCXXB47_XP1_pellet.SoupX.SingCellaR.rdata")
