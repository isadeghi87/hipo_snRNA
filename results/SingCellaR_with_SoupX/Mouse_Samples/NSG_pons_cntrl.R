...
my.umi<-readRDS("../../../cellranger_results/mouse/K35R-CGHTYL/NSG_pons_cntrl.soupX.adjusted.rds")
colnames(my.umi)<-paste(colnames(my.umi),"NSG_pons_cntrl",sep="_")

sce<-SingleCellExperiment(assays = list(counts = my.umi))
mouse <-new("SingCellaR",sce)
mouse@sample_uniq_id<-"NSG_pons_cntrl"
...
save(mouse,file="NSG_pons_cntrl.SoupX.SingCellaR.rdata")
