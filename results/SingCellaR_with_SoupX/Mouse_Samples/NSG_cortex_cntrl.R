...
my.umi<-readRDS("../../../cellranger_results/mouse/K35R-A39LMU/NSG_cortex_cntrl.soupX.adjusted.rds")
colnames(my.umi)<-paste(colnames(my.umi),"NSG_cortex_cntrl",sep="_")

sce<-SingleCellExperiment(assays = list(counts = my.umi))
mouse <-new("SingCellaR",sce)
mouse@sample_uniq_id<-"NSG_cortex_cntrl"
...
save(mouse,file="NSG_cortex_cntrl.SoupX.SingCellaR.rdata")
