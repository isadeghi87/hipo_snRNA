...
my.umi<-readRDS("../../../cellranger_results/human/K35R-4DZ4CT/ICGC_GBM60.soupX.adjusted.rds")
colnames(my.umi)<-paste(colnames(my.umi),"ICGC_GBM60",sep="_")

sce<-SingleCellExperiment(assays = list(counts = my.umi))
human <-new("SingCellaR",sce)
human@sample_uniq_id<-"ICGC_GBM60"
...
get_variable_genes_by_fitting_GLM_model(human,mean_expr_cutoff = 0.05,disp_zscore_cutoff = 0.05)
...
runUMAP(human,dim_reduction_method = "pca",n.dims.use = 20,n.neighbors = 20,
        uwot.metric = "euclidean")


identifyClusters(human,n.dims.use = 20,n.neighbors = 40,knn.metric = "euclidean")

plot_umap_label_by_clusters(human,show_method = "louvain",point.size = 1.5)

findMarkerGenes(human,cluster.type = "louvain")

save(human,file="ICGC_GBM60.SingCellaR.rdata")
